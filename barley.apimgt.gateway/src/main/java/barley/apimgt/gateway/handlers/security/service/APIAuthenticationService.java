...
package barley.apimgt.gateway.handlers.security.service;
...
public class APIAuthenticationService {
    ...
    public void invalidateKeys(APIKeyMapping[] mappings) {
    	...
    	// (수정)
//        Cache gatewayCache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
//        								.getCache(APIConstants.GATEWAY_KEY_CACHE_NAME, String.class, String.class);
    ...
    public void invalidateOAuthKeys(String consumerKey, String authorizedUser) {
    	// (수정)
//        Cache cache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
//					.getCache(APIConstants.KEY_CACHE_NAME, String.class, Set.class);
    ...
    public void invalidateResourceCache(String apiContext, String apiVersion, String resourceURLContext, String httpVerb) {
        ...
        try {
            ...
            // (수정)
//            Cache cache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
//									.getCache(APIConstants.RESOURCE_CACHE_NAME, String.class, VerbInfoDTO.class);
            ...
            // (추가) 2017.12.08 - gateway에 캐쉬키를 하나 더 추가했으므로 캐시 하나 더 클리어 
            // (수정)  
//            Cache apiInfoCache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
//					.getCache(APIConstants.API_INFO_CACHE_NAME, String.class, APIInfoDTO.class);
            Cache apiInfoCache = Caching.getCacheManager(APIConstants.API_MANAGER_CACHE_MANAGER)
            						.getCache(APIConstants.API_INFO_CACHE_NAME);
            if (apiInfoCache.containsKey(apiCacheKey)) {
            	apiInfoCache.remove(apiCacheKey);
            }
    ...
    public void invalidateKey(String accessToken) {
        //TODO Review and fix
    	// (수정)
//        Cache keyCache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
//							.getCache(APIConstants.KEY_CACHE_NAME, String.class, String.class);
    ...
    public void invalidateCachedTokens(String[] accessTokens){
        ...
        // (수정)
//        Cache gatewayCache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
//				.getCache(APIConstants.GATEWAY_TOKEN_CACHE_NAME, String.class, String.class);
...
//				.getCache(APIConstants.GATEWAY_TOKEN_CACHE_NAME, String.class, String.class).removeAll(cachedObjects.keySet());
        ...
        for(String tenantDomain : tenantMap.keySet()){
            try{
                ...
                // (수정)
//                Cache tenantGatewayCache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
//        				.getCache(APIConstants.GATEWAY_TOKEN_CACHE_NAME, String.class, String.class);
