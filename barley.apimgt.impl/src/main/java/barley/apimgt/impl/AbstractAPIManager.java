...
import barley.apimgt.api.APIDefinition;
import barley.apimgt.api.APIManagementException;
import barley.apimgt.api.APIManager;
import barley.apimgt.api.APIMgtResourceAlreadyExistsException;
import barley.apimgt.api.APIMgtResourceNotFoundException;
import barley.apimgt.api.BlockConditionNotFoundException;
import barley.apimgt.api.PolicyNotFoundException;
...
import barley.apimgt.impl.definitions.APIDefinitionFromSwagger20;
...
import barley.apimgt.impl.utils.LRUCache;
import barley.apimgt.impl.utils.TierNameComparator;
...
public abstract class AbstractAPIManager implements APIManager {
    ...
    //(오세창) 이 메소드에서 registry에 관련된 초기화를 진행해야 한다.  
    public AbstractAPIManager(String username) throws APIManagementException {
        ...
        try {
            if (username == null) {
            	...
            	//(오세창) registry는 여기에서 가져온다. osgi 서비스로서 DI형태로 가져온다. 
                ...
                this.username= BarleyConstants.REGISTRY_ANONNYMOUS_USERNAME;
                ...
                // 도메인명으로 테넌트 아이디를 가져온다. (테이블명: UM_TENANT)
    ...
    public API getLightweightAPIByUUID(String uuid, String requestedTenantDomain) throws APIManagementException {
        try {
            ...
            GenericArtifact apiArtifact = artifactManager.getGenericArtifact(uuid);
            if (apiArtifact != null) {
                return APIUtil.getAPIInformation(apiArtifact);
            } else {
                handleResourceNotFoundException(
                        "Failed to get API. API artifact corresponding to artifactId " + uuid + " does not exist");
            }
        ...
        try {
            ...
            if (!MultitenantConstants.SUPER_TENANT_DOMAIN_NAME.equals(tenantDomain)) {
                ...
                if (this.tenantDomain != null && !MultitenantConstants.SUPER_TENANT_DOMAIN_NAME.equals(this.tenantDomain)) {
                    ...
                    // (수정) 2017.12.27 registry를 가져올때 userId로 가져와야 한다. 
...
//                            identifier.getProviderName(), MultitenantConstants.SUPER_TENANT_ID);
                    String providerId = APIUtil.replaceEmailDomainBack(identifier.getProviderName());
                    String userName = MultitenantUtils.getTenantAwareUsername(providerId);
                    passRegistry = ServiceReferenceHolder.getInstance().getRegistryService().getGovernanceUserRegistry(
                    		userName, MultitenantConstants.SUPER_TENANT_ID);
                } else {
...
//                      identifier.getProviderName() + RegistryConstants.PATH_SEPARATOR +
    ...
    public Set<String> getAPIVersions(String providerName, String apiName)
        ...
        String apiPath = APIConstants.API_LOCATION + RegistryConstants.PATH_SEPARATOR +
        					APIUtil.replaceEmailDomain(providerName) + RegistryConstants.PATH_SEPARATOR
        ...
        try {
            ...
            if (this.tenantDomain == null || isTenantDomainNotMatching(apiTenantDomain)) {
                ...
                registryType = ServiceReferenceHolder.getInstance().getRegistryService().getGovernanceUserRegistry(
                        BarleyConstants.REGISTRY_ANONNYMOUS_USERNAME, tenantId);
            } else {
    ...
    public List<Documentation> getAllDocumentation(APIIdentifier apiId) throws APIManagementException {
        ...
        try {
            ...
            for (Association association : docAssociations) {
                ...
                // (수정) artifactConfigurations 초기화가 되지 않아 밑에 코드로 수정  
    ...
    public List<Documentation> getAllDocumentation(APIIdentifier apiId,String loggedUsername) throws APIManagementException {
        ...
        try {
            ...
            for (Association association : docAssociations) {
                ...
                } catch (barley.registry.core.secure.AuthorizationFailedException e) {
