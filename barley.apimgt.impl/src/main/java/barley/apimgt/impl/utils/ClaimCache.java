...
public class ClaimCache {
    ...
    public void addToCache(ClaimCacheKey key, UserClaims entry) {
    	// (수정)
//    	Caching.getCachingProvider().getCacheManager().getCache(CLAIM_CACHE_NAME).put(key,entry);
...
//    	return (Cache.Entry)Caching.getCachingProvider().getCacheManager().getCache(CLAIM_CACHE_NAME).get(key);
...
//    	Caching.getCachingProvider().getCacheManager().getCache(CLAIM_CACHE_NAME).removeAll();
