...
import barley.apimgt.impl.dto.APIInfoDTO;
import barley.apimgt.impl.dto.Environment;
import barley.apimgt.impl.utils.APIAuthenticationAdminClient;
...
import barley.identity.application.common.IdentityApplicationManagementException;
import barley.identity.application.common.model.InboundAuthenticationConfig;
import barley.identity.application.common.model.InboundAuthenticationRequestConfig;
import barley.identity.application.common.model.Property;
import barley.identity.application.common.model.ServiceProvider;
import barley.identity.application.mgt.ApplicationManagementService;
import barley.identity.base.IdentityException;
import barley.identity.oauth.OAuthAdminService;
import barley.identity.oauth.cache.OAuthCache;
import barley.identity.oauth.cache.OAuthCacheKey;
import barley.identity.oauth.common.OAuthConstants;
import barley.identity.oauth.config.OAuthServerConfiguration;
import barley.identity.oauth.dto.OAuthConsumerAppDTO;
...
public class APIKeyMgtSubscriberService {
    ...
    public OAuthApplicationInfo createOAuthApplicationByApplicationInfo(OAuthApplicationInfo oauthApplicationInfo)
        ...
        String baseUser = BarleyContext.getThreadLocalCarbonContext().getUsername();
        ...
        try {
        	// (추가) 2018.04.20 - ApplicationMgtUtil에서 사용자 레지스트리에서 권한체크하기 때문에 추가함.
            RegistryService registryService = RegistryServiceHolder.getInstance().getRegistryService();
            PrivilegedBarleyContext.getThreadLocalCarbonContext().setRegistry(RegistryType.USER_GOVERNANCE, 
            ...
            PrivilegedBarleyContext.getThreadLocalCarbonContext().endTenantFlow();
            PrivilegedBarleyContext.getThreadLocalCarbonContext().setUsername(baseUser);
        }
        return null;
    ...
    public void deleteOAuthApplication(String consumerKey)
        ...
        PrivilegedBarleyContext.getThreadLocalCarbonContext().startTenantFlow();
        PrivilegedBarleyContext.getThreadLocalCarbonContext().setTenantId(subscriber.getTenantId(), true);
        PrivilegedBarleyContext.getThreadLocalCarbonContext().setUsername(tenantAwareUsername);

        try {
        	// (추가) 2018.04.20 - ApplicationMgtUtil에서 사용자 레지스트리에서 권한체크하기 때문에 추가함.
            RegistryService registryService = RegistryServiceHolder.getInstance().getRegistryService();
            PrivilegedBarleyContext.getThreadLocalCarbonContext().setRegistry(RegistryType.USER_GOVERNANCE, 
            		registryService.getGovernanceUserRegistry(baseUser));
        	...
        	// (추가) 2017.10.23 - 어플리케이션 패키지에서 삭제하지 않고 oauth서비스 삭제로 변경함.
            // (주석) 2018.04.20 - 위 코드 appMgtService.deleteApplication()로 처리함으로써 필요없어서 주석처리 
//            OAuthAdminService oAuthAdminService = new OAuthAdminService();
//            oAuthAdminService.removeOAuthApplicationData(consumerKey);
    ...
    public String renewAccessToken(String tokenType, String oldAccessToken,
        ...
        String webContextRoot = BarleyUtils.getServerConfiguration().getFirstProperty("WebContextRoot");
    ...
    public void clearOAuthCache(String consumerKey, String authorizedUser) {
        ...
        // indentity.xml 에서 Oauth 항목의 EnableOAuthCache를 false로 했기 때문에 동작하지 않음 
        if (OAuthServerConfiguration.getInstance().isCacheEnabled()) {
