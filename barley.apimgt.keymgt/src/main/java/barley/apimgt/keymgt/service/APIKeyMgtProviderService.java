 ...
 * revoking, etc.
 */
public class APIKeyMgtProviderService {
    ...
    public void removeScopeCache(String[] consumerKeys) {
        if (consumerKeys != null && consumerKeys.length != 0) {
        	// (수정) 캐시수정 
            Cache appCache = Caching.getCacheManager(APIConstants.API_MANAGER_CACHE_MANAGER).getCache(APIConstants.APP_SCOPE_CACHE);
            /*
        	Cache appCache = Caching.getCachingProvider(RegistryConstants.CACHING_PROVIDER).getCacheManager()
        								.getCache(APIConstants.APP_SCOPE_CACHE, String.class, Map.class);
        								*/
